...
const MovieCard = memo(({ movie }) => {
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.08, zIndex: 30, transition: { duration: 0.3 } }}
      onClick={(e) => { e.stopPropagation(); navigate(`/details/${movie.id}`); }}
      className="relative flex-shrink-0 cursor-pointer group rounded-lg overflow-hidden aspect-[2/3] w-full bg-[#1a1a1a] ring-1 ring-white/10"
    >
      <img
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-black via-black/60 to-transparent">
        <button
          onClick={handlePlay}
          className="w-8 h-8 rounded-full bg-white flex items-center justify-center hover:scale-110 transition active:scale-95 mb-2"
        >
