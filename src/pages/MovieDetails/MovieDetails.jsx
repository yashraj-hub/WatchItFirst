...
import { Play, Star, Clock, Calendar, Globe, Award, ChevronRight, Users, ArrowLeft } from 'lucide-react';
  ...
  return (
    <MainLayout>
      ...
      <div className="relative h-[50vh] overflow-hidden">
        {/* Poster bg */}
        <img
          src={`${TMDB_CONFIG.original}${movie?.poster_path}`}
          ...
          >
            {logoPath ? (
              <img
                ...
                alt={movie?.title}
                className="max-h-24 md:max-h-36 mx-auto object-contain drop-shadow-2xl"
              />
            ) : (
      ...
      <div className="relative z-10 bg-black px-4 md:px-16 lg:px-24 py-14 space-y-14">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-bold text-gray-400">
            ...
            <span className="px-2 py-0.5 border border-white/10 text-gray-400 text-xs uppercase tracking-widest rounded">
              {movie?.status}
            </span>
          </div>

          {/* Genres as plain text */}
          <p className="mt-3 text-xs uppercase tracking-widest text-gray-600 font-bold">
        ...
        {/* Synopsis */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500 mb-3 flex items-center gap-2">
            Synopsis <ChevronRight className="w-3 h-3" />
          </h2>
        ...
        <motion.div
          ...
          className="grid md:grid-cols-2 gap-8 border-t border-white/5 pt-10"
        >
          {director && (
            <div>
        ...
        {/* Cast */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500 mb-6 flex items-center gap-2">
            Top Cast <Users className="w-3 h-3" />
          </h2>
        ...
        {productionCompanies.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500 mb-6">
              Production
            </h2>
            <div className="flex flex-wrap items-center gap-8">
              {productionCompanies.map(company => (
                <div key={company.id} className="flex flex-col items-center gap-2 group">
                  <img
                    src={`${TMDB_CONFIG.w500}${company.logo_path}`}
                    alt={company.name}
                    className="h-8 md:h-10 object-contain opacity-50 group-hover:opacity-100 transition-opacity duration-300"
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="grid grid-cols-2 md:grid-cols-3 gap-8 py-10 border-y border-white/5"
        >
          {[
        ...
        {recommendations.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500 mb-8">
              More Like This
            </h2>
